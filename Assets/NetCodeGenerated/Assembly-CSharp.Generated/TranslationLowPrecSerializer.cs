...
{
    [BurstCompile]
    public struct TranslationLowPrecGhostComponentSerializer
    {
        static GhostComponentSerializer.State GetState()
        {
            ...
            if (!s_StateInitialized)
            {
                ...
                {
                    GhostFieldsHash = 5533276925148055071,
                    ...
                    VariantHash = 10137082255859595637,
                    CopyToSnapshot =
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                snapshot.Value_x = (int) math.round(component.Value.x * 10);
                snapshot.Value_y = (int) math.round(component.Value.y * 10);
                snapshot.Value_z = (int) math.round(component.Value.z * 10);
            }
        }
        [BurstCompile]
        [MonoPInvokeCallback(typeof(GhostComponentSerializer.CopyToFromSnapshotDelegate))]
        private static void CopyFromSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                var Value_x_Before = snapshotBefore.Value_x * 0.1f;
                var Value_x_After = snapshotAfter.Value_x * 0.1f;
                component.Value.x = math.lerp(Value_x_Before, Value_x_After, snapshotInterpolationFactor);
                snapshotInterpolationFactor = snapshotInterpolationFactorRaw;
                var Value_y_Before = snapshotBefore.Value_y * 0.1f;
                var Value_y_After = snapshotAfter.Value_y * 0.1f;
                component.Value.y = math.lerp(Value_y_Before, Value_y_After, snapshotInterpolationFactor);
                snapshotInterpolationFactor = snapshotInterpolationFactorRaw;
                var Value_z_Before = snapshotBefore.Value_z * 0.1f;
                var Value_z_After = snapshotAfter.Value_z * 0.1f;
                component.Value.z = math.lerp(Value_z_Before, Value_z_After, snapshotInterpolationFactor);
        ...
        private static void CalculateChangeMask(IntPtr snapshotData, IntPtr baselineData, IntPtr bits, int startOffset)
        {
            ...
            changeMask |= (snapshot.Value_z != baseline.Value_z) ? (1u<<2) : 0;
        ...
        private static void ReportPredictionErrors(IntPtr componentData, IntPtr backupData, ref UnsafeList<float> errors)
        {
            ...
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.Value.z - backup.Value.z));
            ++errorIndex;
        }
        private static int GetPredictionErrorNames(ref FixedString512 names)
        {
            ...
            names.Append(new FixedString64("Value.x"));
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32(","));
            names.Append(new FixedString64("Value.y"));
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32(","));
            names.Append(new FixedString64("Value.z"));
            ++nameCount;
            return nameCount;
        }
        #endif
