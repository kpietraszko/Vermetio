...
namespace Unity.NetCode
{
    ...
    public class GhostAuthoringComponent : MonoBehaviour, IDeclareReferencedPrefabs, ISerializationCallbackReceiver
    {
        ...
        [SerializeField] private OverrideValue[] SendForChildrenOverrides;
        ...
        public void OnBeforeSerialize()
        {
            ...
            foreach (var m in ComponentOverrides)
            {
                ...
                if (m.SendToChild != 0)
                    t4.Add(new OverrideValue(index, (ulong)m.SendToChild));
                ++index;
            }
            ...
            SendForChildrenOverrides = t4.ToArray();
        }
        ...
        public void OnAfterDeserialize()
        {
            ...
            if (SendForChildrenOverrides != null)
            {
                foreach (var p in SendForChildrenOverrides)
                    ComponentOverrides[p.OverrideIndex].SendToChild = (int)p.Value;
            }
        }
    }
    ...
    public class GhostAuthoringConversion : GameObjectConversionSystem
    {
        ...
        {
            using (var context =
                new BlobAssetComputationContext<int, GhostPrefabMetaData>(BlobAssetStore, 16, Allocator.Temp))
            {
                Entities.ForEach((GhostAuthoringComponent ghostAuthoring) =>
                {
                    ...
                    if (String.IsNullOrEmpty(ghostAuthoring.prefabId))
                        throw new InvalidOperationException(
                            $"The ghost {ghostAuthoring.gameObject.name} is not a valid prefab, all ghosts must be the top-level GameObject in a prefab. Ghost instances in scenes must be instances of such prefabs and changes should be made on the prefab asset, not the prefab instance");

                    if (!isPrefab &&
                        ghostAuthoring.DefaultGhostMode == GhostAuthoringComponent.GhostMode.OwnerPredicted &&
                        target != NetcodeConversionTarget.Server)
                        throw new InvalidOperationException(
                            $"Cannot convert a owner predicted ghost {ghostAuthoring.Name} as a scene instance");

                    if (!isPrefab && DstEntityManager.World.GetExistingSystem<ClientSimulationSystemGroup>() != null)
                        throw new InvalidOperationException(
                            $"The ghost {ghostAuthoring.gameObject.name} cannot be created on the client, either put it in a sub-scene or spawn it on the server only");
                    ...
                    if (target != NetcodeConversionTarget.Client)
                    {
                        ...
                        // this at runtime
                        DstEntityManager.AddSharedComponentData(rootEntity,
                            new SharedGhostTypeComponent { SharedValue = ghostType });
                    }
                    ...
                    if (target != NetcodeConversionTarget.Client || ghostAuthoring.SupportedGhostModes !=
                        GhostAuthoringComponent.GhostModeMask.Interpolated)
                    ...
                    var allComponents = new NativeList<ComponentType>(rootComponents.Length * linkedEntities.Length,
                        Allocator.Temp);
                    ...
                    for (int k = 0; k < linkedEntities.Length; ++k)
                    {
                        ...
                        for (int i = 0; i < numComponents; ++i, ++compIdx)
                        {
                            ...
                            var disableModifier = allComponents[compIdx].GetManagedType()
                                .GetCustomAttribute<DontSupportPrefabOverrides>();
                            ...
                            if (modifier != null)
                            {
                                ...
                                var attr = GetVariantGhostAttribute(allComponents[compIdx].GetManagedType(),
                                    modifier.ComponentVariant);
                                ...
                                if (modifier.OwnerPredictedSendType !=
                        ...
                        for (int i = 0; i < allComponents.Length; ++i)
                        {
                            ...
                            if ((prefabType & GhostPrefabType.Server) == 0)
                            {
                                ...
                                if (typeof(ICommandData).IsAssignableFrom(comp.GetManagedType()))
                                    Debug.LogWarning(
                                        $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on client ghosts. Will be removed from from the server target");
                            }
                        }
                    }
                        ...
                        for (int i = 0; i < allComponents.Length; ++i)
                        {
                            ...
                            if (typeof(ICommandData).IsAssignableFrom(comp.GetManagedType()))
                            {
                                if ((prefabType & GhostPrefabType.Client) == 0)
                                    Debug.LogWarning(
                                        $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on the server. Will be removed from from the client target");
                                else if (ghostAuthoring.SupportedGhostModes ==
                                         GhostAuthoringComponent.GhostModeMask.Predicted &&
                                         (prefabType & GhostPrefabType.PredictedClient) == 0)
                                    Debug.LogWarning(
                                        $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on interpolated ghost. Will be removed from the client target");
                            }
                            ...
                            if ((prefabType & GhostPrefabType.Client) == 0)
                            {
                                ...
                                removedFromClient[i] = true;
                                     ...
                                     GhostAuthoringComponent.GhostModeMask.Interpolated &&
                                     (prefabType & GhostPrefabType.InterpolatedClient) == 0)
                            {
                                DstEntityManager.RemoveComponent(entities[i], comp);
                                removedFromClient[i] = true;
                            }
                    ...
                    else if (ghostAuthoring.SupportedGhostModes == GhostAuthoringComponent.GhostModeMask.Predicted)
                    {
                        for (int i = 0; i < allComponents.Length; ++i)
                        {
                            ...
                            if ((prefabType & (GhostPrefabType.PredictedClient | GhostPrefabType.Server)) == 0)
                            {
                                ...
                                if (typeof(ICommandData).IsAssignableFrom(comp.GetManagedType()))
                                    Debug.LogWarning(
                                        $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on interpolated ghost. Will be removed from the client and server target");
                            }
                        }
                    }
                    else
                    ...
                    if (target != NetcodeConversionTarget.Server)
                    {
                        ...
                        for (int i = 0; i < allComponents.Length && !hasBuffers; ++i)
                            hasBuffers |= (allComponents[i].IsBuffer && !removedFromClient[i]) &&
                                          (prefabTypes[i] & GhostPrefabType.Client) != 0;
                    ...
                    if (isPrefab)
                    {
                        ...
                        contentHash = TypeHash.CombineFNV1A64(contentHash,
                            TypeHash.FNV1A64((int)ghostAuthoring.SupportedGhostModes));
                        contentHash = TypeHash.CombineFNV1A64(contentHash,
                            TypeHash.FNV1A64((int)ghostAuthoring.DefaultGhostMode));
                        contentHash = TypeHash.CombineFNV1A64(contentHash,
                            TypeHash.FNV1A64((int)ghostAuthoring.OptimizationMode));
                        ...
                        var blobHash = new Unity.Entities.Hash128(ghostType.guid0 ^ (uint)(contentHash >> 32),
                            ghostType.guid1 ^ (uint)(contentHash), ghostType.guid2, ghostType.guid3);
                        ...
                        if (context.NeedToComputeBlobAsset(blobHash))
                        {
                            ...
                            if (ghostAuthoring.SupportedGhostModes ==
                                GhostAuthoringComponent.GhostModeMask.Interpolated)
                            ...
                            else if (ghostAuthoring.DefaultGhostMode ==
                                     GhostAuthoringComponent.GhostMode.OwnerPredicted)
                            {
                                if (!DstEntityManager.HasComponent<GhostOwnerComponent>(rootEntity))
                                    throw new InvalidOperationException(
                                        "OwnerPrediction mode can only be used on prefabs which have a GhostOwnerComponent");
                            ...
                            root.StaticOptimization = (ghostAuthoring.OptimizationMode ==
                                                       GhostAuthoringComponent.GhostOptimizationMode.Static);
                            ...
                            var removeOnServer =
                                new NativeList<GhostPrefabMetaData.ComponentReference>(allComponents.Length,
                                    Allocator.Temp);
                            var removeOnClient =
                                new NativeList<GhostPrefabMetaData.ComponentReference>(allComponents.Length,
                                    Allocator.Temp);
                            var disableOnPredicted =
                                new NativeList<GhostPrefabMetaData.ComponentReference>(allComponents.Length,
                                    Allocator.Temp);
                            var disableOnInterpolated =
                                new NativeList<GhostPrefabMetaData.ComponentReference>(allComponents.Length,
                                    Allocator.Temp);

                            // Snapshot data buffers should be removed from the server, and shared ghost type from the client
                            removeOnServer.Add(new GhostPrefabMetaData.ComponentReference(0,
                                TypeManager.GetTypeInfo(ComponentType.ReadWrite<SnapshotData>().TypeIndex)
                                    .StableTypeHash));
                            removeOnServer.Add(new GhostPrefabMetaData.ComponentReference(0,
                                TypeManager.GetTypeInfo(ComponentType.ReadWrite<SnapshotDataBuffer>().TypeIndex)
                                    .StableTypeHash));
                            removeOnClient.Add(new GhostPrefabMetaData.ComponentReference(0,
                                TypeManager.GetTypeInfo(ComponentType.ReadWrite<SharedGhostTypeComponent>().TypeIndex)
                                    .StableTypeHash));
                            if (hasBuffers)
                                removeOnServer.Add(new GhostPrefabMetaData.ComponentReference(0,
                                    TypeManager.GetTypeInfo(ComponentType.ReadWrite<SnapshotDynamicDataBuffer>()
                            ...
                            if (ghostAuthoring.SupportedGhostModes == GhostAuthoringComponent.GhostModeMask.All)
                                disableOnInterpolated.Add(new GhostPrefabMetaData.ComponentReference(0,
                                    TypeManager.GetTypeInfo(
                                        ComponentType.ReadWrite<PredictedGhostComponent>().TypeIndex).StableTypeHash));
                            else if (ghostAuthoring.SupportedGhostModes ==
                                     GhostAuthoringComponent.GhostModeMask.Interpolated)
                                removeOnClient.Add(new GhostPrefabMetaData.ComponentReference(0,
                                    TypeManager.GetTypeInfo(
                            ...
                            compIdx = 0;
                            var blobNumServerComponentsPerEntity =
                                builder.Allocate(ref root.NumServerComponentsPerEntity, linkedEntities.Length);
                            for (int k = 0; k < linkedEntities.Length; ++k)
                            {
                                ...
                                for (int i = 0; i < numComponents; ++i, ++compIdx)
                                {
                                    ...
                                    if (isCommandData)
                                    {
                                        ...
                                        if ((prefabType & GhostPrefabType.Server) == 0)
                                            Debug.LogWarning(
                                                $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on the clients. Will be removed from server ghost prefab");
                                        if ((prefabType & GhostPrefabType.Client) == 0)
                                            Debug.LogWarning(
                                                $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on the server. Will be removed from from the client ghost prefab");
                                        else if (prefabType == GhostPrefabType.InterpolatedClient)
                                            Debug.LogWarning(
                                                $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on interpolated ghost. Will be removed from the server and predicted ghost prefab");
                                        //Check the disabled components for potential and reportor warning for some cases
                                        if (ghostAuthoring.SupportedGhostModes ==
                                            GhostAuthoringComponent.GhostModeMask.All)
                                        {
                                            if ((prefabType & GhostPrefabType.InterpolatedClient) != 0 &&
                                                (prefabType & GhostPrefabType.PredictedClient) == 0)
                                                Debug.LogWarning(
                                                    $"{ghostAuthoring.gameObject.name}: ICommandData {comp} is configured to be present only on interpolated ghost. Will be disabled on predicted ghost after spawning");
                                        }
                                    }
                                    ...
                                    if (ghostAuthoring.SupportedGhostModes == GhostAuthoringComponent.GhostModeMask.All)
                                    {
                                        ...
                                        if ((prefabType & GhostPrefabType.InterpolatedClient) == 0 &&
                                            (prefabType & GhostPrefabType.PredictedClient) != 0)
                                            disableOnInterpolated.Add(
                                                new GhostPrefabMetaData.ComponentReference(k, hash));
                                        if ((prefabType & GhostPrefabType.InterpolatedClient) != 0 &&
                                            (prefabType & GhostPrefabType.PredictedClient) == 0)
                            ...
                            if (target != NetcodeConversionTarget.Client)
                            {
                                ...
                                var blobRemoveOnServer =
                                    builder.Allocate(ref root.RemoveOnServer, removeOnServer.Length);
                            ...
                            if (target != NetcodeConversionTarget.Server)
                            {
                                var blobRemoveOnClient =
                                    builder.Allocate(ref root.RemoveOnClient, removeOnClient.Length);
                            ...
                            if (target != NetcodeConversionTarget.Server)
                            {
                                ...
                                var blobDisableOnPredicted = builder.Allocate(ref root.DisableOnPredictedClient,
                                    disableOnPredicted.Length);
                                for (int i = 0; i < disableOnPredicted.Length; ++i)
                                    blobDisableOnPredicted[i] = disableOnPredicted[i];
                                var blobDisableOnInterpolated = builder.Allocate(ref root.DisableOnInterpolatedClient,
                                    disableOnInterpolated.Length);
                        ...
                        DstEntityManager.AddComponentData(rootEntity,
                            new GhostPrefabMetaDataComponent { Value = blob });
        ...
        {
            if (variantHash != 0 &&
                GhostAuthoringModifiers.VariantsCache.TryGetValue(componentType.FullName, out var variantList))
            {
